...
abstract class BaseRepository implements BaseRepositoryInterface
{
    ...
    public function joinClosure($join, Closure $closure)
    {
        $this->model = $this->model->join($join, $closure);
        return $this;
    }
    ...
    public function createOrUpdateOneToManyRelation($model,$relation, $column, $dataToAttched,$isUpdate)
    {
        $records = [];
        
        if($isUpdate)
        {
            $oldData = $model->$relation->pluck($column)->toArray();
            $dataToDetached=array_diff($oldData,$dataToAttched);
            $dataToAttched=array_diff($dataToAttched,$oldData);
            !empty($dataToDetached)?$model->$relation()->whereIn($column,$dataToDetached)->delete():null;
        }
        if(!empty($dataToAttched)){

            foreach($dataToAttched as $recordId)
            {
                $records[] = [$column => $recordId];
            }
            $model->$relation()->createMany($records);
        }
    }
